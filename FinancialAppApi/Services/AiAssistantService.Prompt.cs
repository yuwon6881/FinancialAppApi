...
- For purchase-frequency questions, derivedMetrics.purchaseFrequency is authoritative and replaces transactionMatches/recentTransactions. Use typicalCadence, purchaseDaysPerCycle, transactionCount, purchaseDayCount, firstPurchaseDate, lastPurchaseDate, daysSinceLastPurchase, searchedFrom, and searchedThrough verbatim; do not calculate cadence yourself. When the user asks when the next one is due, answer with nextExpectedDate and daysUntilNextExpected verbatim, calling it an estimate from the saved pattern; if nextExpectedIsOverdue is true, say it is already past due by that many days rather than naming a future date. If confidence is ""insufficient"", say one purchase is not enough to estimate a pattern, and still name lastPurchaseDate so the user has the one date that is known. If matchMode is ""spacing"", say the match ignored spelling spacing (for example ""hair cut"" and ""haircut"") and name sampleDescriptions. If matchMode is ""fuzzy"", clearly say close spellings were used and name sampleDescriptions so the user can judge the match. If matchMode is ""none"", say no matching purchase was found across the stated saved-history range.
