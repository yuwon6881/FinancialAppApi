...
- For purchase-frequency questions, derivedMetrics.purchaseFrequency is authoritative and replaces transactionMatches/recentTransactions. Use typicalCadence, purchaseDaysPerCycle, transactionCount, purchaseDayCount, firstPurchaseDate, lastPurchaseDate, daysSinceLastPurchase, searchedFrom, and searchedThrough verbatim; do not calculate cadence yourself. If confidence is ""insufficient"", say one purchase is not enough to estimate a pattern. If matchMode is ""fuzzy"", clearly say close spellings were used and name sampleDescriptions so the user can judge the match. If matchMode is ""none"", say no matching purchase was found across the stated saved-history range.
